...
export default function SecondPage() {
  ...
  useEffect(() => {
    api.get("/secondpage/get_many", {
      ...
      .then((res) => {
        ...
        setTotalV(res.data.map(item => item.valor).reduce((total, item) => total + item), 0)
        setTotalQ(res.data.map(item => item.quantidade).reduce((total, item) => total + item), 0)
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  const deleteItem = (id) => {
    api.delete(`/secondpage/delete/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
  ...
  return (
    ...
    <div className="teste">
      ...
      <div className="container">
        ...
        <div className="left">
          ...
          <div className="modaltest">
            <button className="transaction-button" onClick={() => setShow(true)} onclick="showModal()">Nova transação</button>
          ...
          <div className="backk">
            ...
            <Link to="/">
              <button className="back-button" >
                <IoMdArrowBack size={16} color="#fff" /></button>
            </Link>
          </div>


          <div className="darkk">
            <button className="darkmode-button" onClick={darkmode}> <MdDarkMode size={16} color="#fff" /></button>
          </div>
              ...
              {purchases.map((purchase) => {
                return (
                  <tr key={purchase._id}>
                    ...
                    <td>{`${purchase.data.slice(5, 7)}/${purchase.data.slice(8, 10)}/${purchase.data.slice(0, 4)}`}</td>
                    <td id="remove">
      ...
      <EditModal onClose={() => setShoww(false)} showw={showw} id={id} />
    </div>
  );
}